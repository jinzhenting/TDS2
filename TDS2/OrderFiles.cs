...
namespace TDS2
{
    ...
    public static class OrderFiles
    {
        ...
        public static List<string> Get(DataRow orderRow)
        {
            ...
            string embZ = orderRow["打版师"].ToString();// 打版师
            ...
            {
                foreach (string datas in FileSystem.Data12())// 3个Data文件夹共12个子文件夹列表// Dst文件路径 和Emb分开遍历，是避免Dst和Emb不储存在同一个Data当中
                {
                ...
                /// 在 myAttachIn 中查找图片
                string attachInPath = Path.Combine(FileSystem.MyAttach(), "Attach_In");
                ...
                /// 在 myAttachOut 中查找图片
                string attachOutPath = Path.Combine(FileSystem.MyAttach(), "Attach_Out");
            ...
            {
                /// 在 打版师文件夹 中查找图片
                string zFlie = Path.Combine(FileSystem.ZFile(), embZ, "Jpg_Dst");// 在打版师文件夹中查找图片
            ...
            {
                string todayPath = Path.Combine(FileSystem.Vector(), "Today");// Vector\Today中查找
                ...
                ///
                string vectorPath = Path.Combine(FileSystem.Vector(), OrderNameParser.GetFilePath(orderName, "Vector"));// Vector\VcetorData中查找
